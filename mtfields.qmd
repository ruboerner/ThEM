...
## Impedance and apparent resitivity
...
On the surface of a halfspace, we can measure the _surface impedance_
$$
Z_s = Z(\omega),
$$
which in case of a uniform halfspace is identical to the _intrinsic impedance_
$$
...
From $Z_s$ we can infer the _apparent resistivity_
$$
...
For our example, the apparent resistivity is `{python} rhoa(Zs, f)` $\Omega \cdot m$.
The phase angle is `{python} phi(Zs)` degrees.
...
This can be visualized using _sounding curves_ of $\rho_a$ vs. period $T = 1/f$ or $\varphi$ vs. $T$.
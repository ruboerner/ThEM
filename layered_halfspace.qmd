...
First, we formulate a boundary value problem for the potential inside the layer $n$, $1 < n < N$, and extend its solution upwards to the layer $n-1$ by applying appropriate conditions of continuity of the fields and the potential.

In what follows, we only consider that part of the solution of the Helmholtz equation that only depends on $z$, i.e., $V(z, \lambda)$.

Inside layer $n$ we define the function $\lambda_n = \sqrt{\lambda^2 + i\omega\mu_0 \sigma_n}$ where $h_n < z < h_{n+1}$, $1 \le n \le N$.
...
&=\gamma_{n}^{-}\left[e^{-\lambda_{n}\left(z-h_{n}\right)}+r_{n} e^{+\lambda_{n}\left(z-h_{n}\right)}\right].
\end{aligned}
\end{equation}
$$
...
Note that $r_n := \dfrac{\gamma_n^+}{\gamma_n^-}$ is the ratio of the amplitudes of upward (reflected) and downward (transmitted) travelling waves in the interace at $z=h_n$.
...
Let's first study the wave propagation in layer $N$.
...
In the case of a layered halfspace, this can be assumed for any $z=h_n$, $ 1 \le n \le N$.
...
To this end, we define the _admittance_ of the wave by setting
$$
\begin{equation}
...
Later we can derive the _surface impedance_ $Z_1$, with the help of which we infer other quantities such as the _apparent resistivity_ and _phase_.
...
In the following we introduce some auxiliary functions like the _reflection coefficient function_  $r_n(\lambda)$.

### Behaviour of $V(z)$ in layer $N$

With $r_n$ we describe the ratio of upward and downward travelling partial waves.

In $z=h_N$ we don't observe an upward travelling wave, i.e., there is no reflection arising from a further interface below $z=h_N$.

Therefore, $r_N = 0$  in $z=h_N$ for all $\lambda$.

The substratum acts like a homogeneous halfspace.

## Implementation

In the following we want to derive a formulation for the computation of $B_1$.
...
- `hₙ`: $z$-coordinate of top of layer $n$
...
```
Further, we define `V(z)` as a symbolc expression for $V(z)$:
```{python}
...
\end{equation} 
$$ {#eq-Bn}
We now have a representation of the admittance in terms of the layer properties $\lambda_n$ and the reflection coefficient $r_n$.

Yet, the $r_n$ is still undetermined. We try to eliminate $r_n$ from the equation above.
...
In `sympy` we carry out the following steps:

- Evaluate $B(z)$ in $z=h_{n+1}$ by
- Substituting (`subs`) $h_{n+1} - h_n$ by $d_n$
...
r[0]
```
Given this intermediate result, we are now prepared to substitute the expression $r_n$ in the equation @eq-Bn introduced above for $B_n$.
...
With the following (cosmetic) changes we obtain a recursion formula which provides the desired surface admittance $B_1$.
...
We are now able to express the effect of the layered halfspace by the _surface admittance_ $B_1$.
...
plt.plot([-nc0 + v for v in range(1, nc+1)], fc0)
```
...
plt.loglog(freq, np.abs(np.real(Bz[10, :])) / mu0, label='Re')
plt.loglog(freq, np.abs(np.imag(Bz[10, :])) / mu0, label='Im')
plt.grid(True)
plt.legend()
plt.xlabel('f in Hz')
plt.ylabel(r'$H_z$ in A/m')
plt.title(r'$\sigma = $' + str(sigma[0]) + ' S/m, r = ' + '{0:.2f}'.format(rr[10]) + ' m')
plt.show()
```
...
plt.loglog(freq, np.abs(np.real(H)), label='Re')
plt.loglog(freq, np.abs(np.imag(H)), label='Im')
plt.grid(True)
plt.legend()
plt.xlabel('f in Hz')
plt.ylabel(r'$H_z$ in A/m')

...
To this end, we have to implement a discrete convolution. 
...
ax.loglog(freq, np.abs(np.real(Bz[10, :])) / mu0, label='Re')
ax.loglog(freq, np.abs(np.imag(Bz[10, :])) / mu0, label='Im')
ax.grid(True)
ax.legend()
...
ax.loglog(freq, np.abs(np.real(H)), label='Re')
ax.loglog(freq, np.abs(np.imag(H)), label='Im')
ax.grid(True)
ax.legend()
ax.set_xlabel('f in Hz')
ax.set_ylabel(r'$H_z$ in A/m')

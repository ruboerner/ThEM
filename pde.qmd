...
Until now, we have not considered the conductivities in that region. This can be studied with the ratio of both coefficients.
We want to show that
$$
...
In the last example we have used a conductivity of $10^{-5}$ S/m as a reasonable low value for minerals.
We recognize that the ratio is large for frequencies less than $10^5$ Hz.

To conclude, we can safely assume that the second term, $\eta_2$, and correspondingly the term $\mu\varepsilon$, can be neglected for frequencies less than $10^5$ Hz for the lowest reasonable value of the conductivity in the Earth.
For higher conductivities, the result becomes even more pronounced.
...
$$
 \frac{1}{c_0^2} \to 0,
$$
which violates the generally recognized fact that the speed of light is finite.
On the other hand, this makes it clear that changes in the fields happen everywhere at the same time.
...
$$
and use the properties of the Fourier transform to equivalently formulate time derivatives.
It holds that
$$
...
H_x(z) = H_x(0) e^{-i k z}.
$$ {#eq-solution-h}
The propagation inside the Earth depends on $k^2$. However, in @eq-solution-h we need the root of $k^2$. 
...
We  model the field component $H_x(z)$ as a function of $\alpha$ and $\beta$.
With $k = \pm (\alpha - i\beta)$ we get
$$
...
The following animation shows the real and imaginary part of the magnetic field for a frequency of 1 Hz in a uniform halfspace with a resistivity of 100 $\Omega \cdot m$.
The red and green horizontal lines indicate the skin depth and the wave length, resp.

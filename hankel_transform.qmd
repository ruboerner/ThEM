...
The choice of these interpolation points depends on the source-receiver offsets $r$ for which the integral is to be calculated.
If the offsets $r_i$ are also logarithmically equidistant, the convolution sum can use all values of $r_i$ in one pass.
...
The number of required function evaluations of $f$ is $n_c + n_r - 1$.
The location of the discrete $\lambda_n$ depends on the minimum spacing $r_{min}$.
...
Unlike our example, we usually provide logarithmically equidistant distances.
Source and receiver points are both in the plane $z=0$.
The Euclidean distance between the two is denoted by $R$.
...
We calculate the field values for several logarithmically equidistant frequencies at once.
For our example we choose $0.1 < f < 10^5$ Hz.
...
for k, freq in enumerate(f):
    ...
    for nu in range(0, ncnr):
        ...
        # B1 = admittance(lam, [sigma], [mu_0], [], freq)
...
```{python}
res = Bz_ana - Bz
print(np.linalg.norm(res) / np.linalg.norm(Bz_ana))
```

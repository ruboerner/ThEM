...
Start from the Hankel transform

$$
g(r) = \int_0^\infty f(\lambda)\, J_0(\lambda r)\, \dd\lambda .
$$

Introduce logarithmic variables

$$
x = \log\left(\frac{r}{r_0}\right),
\qquad
y = -\log(\lambda r_0).
$$
...
#### Transform the variables {.unnumbered}
...
Now compute the argument of the Bessel function:

$$
\lambda r = 
\left(\frac{e^{-y}}{r_0}\right)
(r_0 e^x) = 
e^{x-y}.
$$
...
#### Transform the differential {.unnumbered}
...
#### Transformation of the integration limits {.unnumbered}
...
Now examine the transformed variable:

* If $\lambda \to 0^+$,

$$
y = -\log(\lambda r_0) \to +\infty.
$$
...
#### Substitute into the integral {.unnumbered}

$$
g(r_0 e^x)
=
\int_{+\infty}^{-\infty}
f\left(\frac{e^{-y}}{r_0}\right)
J_0(e^{x-y})
\left(
-\frac{e^{-y}}{r_0}
\right)
...
Now reverse the limits to remove the minus sign:
...
#### Identify the convolution structure {.unnumbered}
...
This is a convolution in logarithmic coordinates. The key point is that the kernel depends only on the difference

$$
x-y,
$$

which enables FFT-based acceleration in the Fast Hankel Transform.

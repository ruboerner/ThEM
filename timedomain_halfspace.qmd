...
Plot of the filter coefficients:

```{python}
fc, nc, nc0 = gethankelfcsin()
plt.plot([nc0 - i for i in range(0, nc)], fc)
plt.grid(True)
```

In the following we implement a function for the computation of the step response of a layered halfspace.
...
```{python}
plt.loglog(t, np.abs(dBzdt), marker='o', label='FHT')
plt.loglog(t, np.abs(dBzdt_ana), label='analytical')
plt.legend()
plt.xlabel('t in s')
plt.ylabel(r'$db_z(t)/dt \mathrm{~in~} V/m^2$')
plt.grid(True)
